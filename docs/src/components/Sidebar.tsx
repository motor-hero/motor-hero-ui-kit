...
const navigation = [
	...
	},
	{
		...
		items: [
			...
			{ id: "base-dialog", label: "BaseDialog" },
	...
	},
	{
		category: "Hooks",
		items: [
			{ id: "hooks", label: "useDisclosure() / useTheme() / useCustomToast()" },
		],
	},
	{
						...
						{group.items.map((item) => (
							<li key={item.id}>
								<button
									...
									className={`w-full rounded-md px-3 py-1.5 text-left text-sm transition-all duration-200 ${currentPage === item.id
